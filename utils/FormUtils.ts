...
import { VisitorIdUtils } from './VisitorUtils'
  ...
  static async submitFeedback(email: string, advice: string): Promise<boolean> {
    ...
    try {
      const response = await axios.get(
        `https://script.google.com/macros/s/AKfycbxJUXMY2za1l0ENM-3jOk5m5ewQMt_8vHbsi5OlwtYzfE8uudXQ8P5teFRvTg8C6aXyxQ/exec?action=insert&table=tab_final&data=${finalData}`
      )
